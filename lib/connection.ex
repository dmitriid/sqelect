if Code.ensure_loaded?(Sqlitex.Server) do
  defmodule Sqelect.Connection do
    @moduledoc false
    ...
    def child_spec(opts) do
      ...
      DBConnection.child_spec(Sqelect.DbConnection.Protocol, opts)
    end

    def ddl_logs(_), do: []

    def delete(prefix, table, filters, returning) do
      {filters, _} = intersperse_reduce(
        filters,
        " AND ",
        1,
        fn field, acc ->
    ...
    def execute(conn, sql, params, opts) when is_binary(sql) or is_list(sql) do
      query = %Sqelect.DbConnection.Query{name: "", statement: IO.iodata_to_binary(sql)}
      case DBConnection.prepare_execute(conn, query, map_params(params), opts) do
        {:ok, %Sqelect.DbConnection.Query{}, result} ->
          {:ok, result}
        {:error, %Sqelect.DbConnection.Error{}} = error ->
          error
        {:error, err} ->
          raise err
        other ->
    ...
    def prepare_execute(conn, name, sql, params, opts) do
      query = %Sqelect.DbConnection.Query{name: name, statement: sql}
      case DBConnection.prepare_execute(conn, query, map_params(params), opts) do
        {:ok, _, _} = ok ->
          ok
        {:error, %Sqelect.DbConnection.Error{}} = error ->
    ...
    def query(conn, sql, params, opts) do
      query = %Sqelect.DbConnection.Query{statement: sql}
    ...
    def stream(conn, sql, params, opts) do
      %Sqelect.DbConnection.Stream{conn: conn, query: sql, params: params, options: opts}
    end
    ...
    def update(prefix, table, fields, filters, returning) do
      {fields, count} = intersperse_reduce(
        fields,
        ", ",
        1,
        fn field, acc ->
      ...
      {filters, _count} = intersperse_reduce(
        filters,
        " AND ",
        count,
        fn field, acc ->
    ...
    defp create_names(as_prefix, sources, pos, limit, stmt) when pos < limit do
      incoming_prefix = if as_prefix == nil do
        []
      else
        as_prefix
      end
      current =
        case elem(sources, pos) do
          {:fragment, _, _} ->
            {nil, incoming_prefix ++ [?f | Integer.to_string(pos)], nil}
          {table, schema, prefix} ->
            name = incoming_prefix ++ [String.first(table) | Integer.to_string(pos)]
            {quote_table(prefix, table), name, schema}
          %Ecto.SubQuery{} ->
            {nil, [?s | Integer.to_string(pos)], nil}
        end
      [current | create_names(incoming_prefix, sources, pos + 1, limit, stmt)]
    end
    ...
    defp map_params(params) do
      Enum.map params, fn
        %{__struct__: _} = data_type ->
          ## TODO: Is this a proper replacement for  Ecto.DataType.dump?
          {:ok, value} = Ecto.Type.dump(data_type, Ecto.Type.type(data_type))
          value
        %{} = value ->
    ...
    defp default_expr({:ok, %{} = map}, :map) do
      default = Application.get_env(:ecto, :json_library).encode!(map)
